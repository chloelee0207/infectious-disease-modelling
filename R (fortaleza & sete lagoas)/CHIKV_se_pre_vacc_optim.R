...
# Population by age group is recorded for 2022 in the spreadsheet. The Sete Lagoas
# outbreak we model runs over 2023-2024, so we grow the 2022 age-group counts to
# 2023 using the town-level exponential growth rate:
#   r = ln(pop_2025 / pop_2022) / (years between)  [per year]
#   pop_2023 = pop_2022 * exp(r * 1)               [1 year on from 2022]
# The growth rate is applied uniformly to every age group (counts scale, the
# ever-infected proportions are unchanged).
pop_2022_total <- 227397
pop_2025_total <- 238909
growth_r       <- log(pop_2025_total / pop_2022_total) / 3   # per-year growth rate
N           <- age_df$pop_num * exp(growth_r * 1)            # 2022 -> 2023 counts
R_init_prop <- age_df$prop_ever_infected
A           <- nrow(age_df)
cat(sprintf("Population growth rate r = %.5f /yr; total pop 2022 = %d -> 2023 = %d\n",
...
# estimate it. rho = 0.45 implies ~54% attack rate among susceptibles, typical for
...
# We now capture the outbreak FROM ITS START (2023-W40, the first infectious cases),
...
# constant (flat) afterwards. This kills the spline tail curl-up. The outbreak has
# essentially ended by ~2024-W21/22, beyond which beta is unidentified.
#   2023-W40 = index 1 ; 2024-W01 = index 14 ; 2024-W21 = index 34 ; 2024-W22 = index 35.
# Set active_weeks <- T_weeks (52) to recover a full-window spline.
active_weeks <- 30   # = 2024-W17 (near beta minimum)
...
# ------------------------------------------------------------
weeks    <- 1:T_weeks
peak_idx <- which.max(observed_cases)              # case peak (~week 18, 2024-W05)
...
# basis. No Hyolim reference is used: the case curve rises from the 2023-W40 seed to
...
# 7. Plots
# ------------------------------------------------------------
# Shared x-axis ticks for the 2023-W40 -> 2024-W39 window: week 40, 50 (2023),
# the 2024 boundary, then week 10, 20, 30 (2024).
x_ticks <- sete_obs |>
  filter((Year == 2023 & week %in% c(40, 50)) |
         (Year == 2024 & week %in% c(10, 20, 30)))
year_break <- mean(c(max(sete_obs$week_index[sete_obs$Year == 2023]),
                     min(sete_obs$week_index[sete_obs$Year == 2024])))

# (a) Fitted beta_t (base-R quick look)
par(mfrow = c(2, 1), mar = c(4, 4, 2, 1))
plot(weeks, best_beta_t, type = "l", lwd = 2, col = "#d6604d",
     xlab = "Week", ylab = expression(beta[t]),
     main = "Fitted Sete Lagoas beta_t (2023-W40 to 2024-W39)",
     xaxt = "n")
...
plot(weeks, observed_cases, type = "p", pch = 16, cex = 0.6, col = "black",
     ...
     main = "Sete Lagoas: predicted vs observed reported cases", xaxt = "n")
axis(1, at = x_ticks$week_index, labels = x_ticks$week)
...
ggplot(plot_df, aes(x = week)) +
  ...
  annotate("text", x = year_break, y = 0, label = "2024", angle = 90,
  ...
  labs(x = "Week",
       ...
       title = "CHIKV cases in Sete Lagoas (2023-2024)") +
...
ggplot(beta_df, aes(x = week)) +
  ...
  geom_vline(xintercept = year_break, linetype = "dashed", colour = "grey50") +
  annotate("text", x = year_break, y = min(beta_df$beta_lo), label = "2024",
           angle = 90, vjust = -0.5, hjust = -11.5, fontface = "bold", size = 3.5) +
  scale_x_continuous(breaks = x_ticks$week_index, labels = x_ticks$week) +
  labs(x = "Week",
       y = expression(beta[t]),
       title = "Fitted weekly transmission rate for Sete Lagoas (2023-2024)") +
  theme_bw(base_size = 12) +
